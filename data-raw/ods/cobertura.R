...
sub_registro_sinasc_muni <- rbind(
...
sub_registro_sinasc_muni$sub_notificacao_ms_sinasc <- round(as.numeric(sub_registro_sinasc_muni$sub_notificacao_ms_sinasc), 1)

sub_registro_sinasc_muni <- sub_registro_sinasc_muni |>
  dplyr::mutate(
...
sub_registro_sim_muni$sub_notificacao_ms <- round(as.numeric(sub_registro_sim_muni$sub_notificacao_ms), 1)

sub_registro_sim_muni <- sub_registro_sim_muni |>
  dplyr::mutate(
    cobertura = 100 - sub_notificacao_ms
  )

#Juntando as bases referentes aos municípios
base_cobertura_muni <- dplyr::full_join(
  sub_registro_sinasc_muni |> dplyr::rename(cobertura_sinasc = cobertura),
  sub_registro_sim_muni |> dplyr::rename(cobertura_sim = cobertura),
...
sub_registro_sinasc_uf_regioes <- rbind(
...
sub_registro_sinasc_uf_regioes$sub_notificacao_ms <- round(as.numeric(sub_registro_sinasc_uf_regioes$sub_notificacao_ms), 1)

sub_registro_sinasc_uf_regioes <- sub_registro_sinasc_uf_regioes |>
  dplyr::mutate(
    cobertura = 100 - sub_notificacao_ms
  )

sub_registro_sinasc_uf_regioes$localidade[which(sub_registro_sinasc_uf_regioes$localidade == "Total Brasil")] <- "Brasil"
...
sub_registro_sim_uf_regioes <- rbind(
...
sub_registro_sim_uf_regioes$sub_notificacao_ms <- round(as.numeric(sub_registro_sim_uf_regioes$sub_notificacao_ms), 1)
sub_registro_sim_uf_regioes$localidade[which(sub_registro_sim_uf_regioes$localidade == "Total Brasil")] <- "Brasil"

sub_registro_sim_uf_regioes <- sub_registro_sim_uf_regioes |>
  dplyr::mutate(
    cobertura = 100 - sub_notificacao_ms
  )

#Juntando as bases referentes às UFs e regiões
base_cobertura_uf_regioes <- dplyr::full_join(
  sub_registro_sinasc_uf_regioes  |> dplyr::rename(cobertura_sinasc = cobertura),
  sub_registro_sim_uf_regioes |> dplyr::rename(cobertura_sim = cobertura),
  by = c("ano", "localidade")
  ) |>
...
#Exportando as bases
usethis::use_data(base_cobertura_muni, overwrite = TRUE)
usethis::use_data(base_cobertura_uf_regioes, overwrite = TRUE)
usethis::use_data(sub_registro_sinasc_muni, overwrite = TRUE)
usethis::use_data(sub_registro_sim_muni, overwrite = TRUE)
usethis::use_data(sub_registro_sinasc_uf_regioes, overwrite = TRUE)
usethis::use_data(sub_registro_sim_uf_regioes, overwrite = TRUE)
